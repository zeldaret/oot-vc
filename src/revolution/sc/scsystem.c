...
const char* __SCVersion = "<< RVL_SDK - SC \trelease build: Sep  7 2006 07:22:06 (0x4200_60422) >>";

static SCControl Control;
static u8 ConfBuf[SC_CONF_MAX_SIZE] ATTRIBUTE_ALIGN(32);
static u8 ConfBufForFlush[SC_CONF_MAX_SIZE] ATTRIBUTE_ALIGN(32);
...
static SCNameAndID NameAndIDTbl[SC_ITEM_MAX] = {
    {"IPL.CB", SC_ITEM_IPL_CB},     {"IPL.AR", SC_ITEM_IPL_AR},     {"IPL.ARN", SC_ITEM_IPL_ARN},
    {"IPL.DH", SC_ITEM_IPL_DH},     {"IPL.E60", SC_ITEM_IPL_E60},   {"IPL.IDL", SC_ITEM_IPL_IDL},
    {"IPL.LNG", SC_ITEM_IPL_LNG},   {"IPL.NIK", SC_ITEM_IPL_NIK},   {"IPL.PC", SC_ITEM_IPL_PC},
    {"IPL.PGS", SC_ITEM_IPL_PGS},   {"IPL.SSV", SC_ITEM_IPL_SSV},   {"IPL.SADR", SC_ITEM_IPL_SADR},
    {"IPL.SND", SC_ITEM_IPL_SND},   {"NET.CNF", SC_ITEM_NET_CNF},   {"NET.CTPC", SC_ITEM_NET_CTPC},
    {"NET.PROF", SC_ITEM_NET_PROF}, {"NET.WCPC", SC_ITEM_NET_WCPC}, {"DEV.BTM", SC_ITEM_DEV_BTM},
    {"DEV.VIM", SC_ITEM_DEV_VIM},   {"DEV.CTC", SC_ITEM_DEV_CTC},   {"DEV.DSM", SC_ITEM_DEV_DSM},
    {"BT.DINF", SC_ITEM_BT_DINF},   {"BT.SENS", SC_ITEM_BT_SENS},   {"BT.SPKV", SC_ITEM_BT_SPKV},
    {"BT.MOT", SC_ITEM_BT_MOT},     {"BT.BAR", SC_ITEM_BT_BAR},     {"DVD.CNF", SC_ITEM_DVD_CNF},
...
static s32 SCReloadConfFileAsync(u8* buf, u32 size, SCAsyncCallback callback) {
    ...
    Control.bufferSizes[SC_CONF_FILE_PRODUCT] = ARRAY_COUNT(OS_SC_PRDINFO);
...
#pragma unused(block)
    ...
    if (result == NAND_RESULT_OK) {
        ...
        if (NANDReadAsync(&Control.fileInfo, Control.fileBuffers[Control.openFileType],
                          Control.bufferSizes[Control.openFileType], ReadCallbackFromReload,
...
_openFile:
    ...
    OS_SC_PRDINFO[ARRAY_COUNT(OS_SC_PRDINFO) - 1] = '\0';
...
bool SCFindU8Item(u8* dst, SCItemID id) { return SCFindIntegerItem(dst, id, SC_ITEM_BYTE); }

bool SCFindS8Item(s8* dst, SCItemID id) { return SCFindIntegerItem(dst, id, SC_ITEM_BYTE); }

bool SCFindU32Item(u32* dst, SCItemID id) { return SCFindIntegerItem(dst, id, SC_ITEM_LONG); }

bool SCReplaceU8Item(u8 data, SCItemID id) { return SCReplaceIntegerItem(&data, id, SC_ITEM_BYTE); }
...
void SCFlushAsync(SCFlushCallback callback) {
    ...
    if (status == SC_STATUS_READY) {
        ...
        if (!__SCIsDirty()) {
            ...
            if (NANDPrivateGetTypeAsync(ConfFileName, &ctrl->fileType, MyNandCallback, &ctrl->commandBlock) !=
                NAND_RESULT_OK) {
                ErrorFromFlush();
            }
        }
    ...
    switch (ctrl->nandCbState) {
        case NAND_CB_STATE_0:
            if (result == NAND_RESULT_OK && ctrl->fileType == NAND_FILE_TYPE_FILE) {
                ...
                if (NANDPrivateGetStatusAsync(ConfFileName, &ctrl->fileAttr, MyNandCallback, &ctrl->commandBlock) !=
                    NAND_RESULT_OK) {
                    goto _error;
                }
            } else {
        ...
        case NAND_CB_STATE_1:
            if (result == NAND_RESULT_OK && ctrl->fileAttr.perm == NAND_PERM_RWALL) {
                goto _case_5_lbl;
            }
        _case_1_lbl:
        ...
        case NAND_CB_STATE_2:
            ...
            if (NANDPrivateGetTypeAsync(ConfDirName, &ctrl->fileType, MyNandCallback, &ctrl->commandBlock) !=
                NAND_RESULT_OK) {
                goto _error;
            }
            return;
        ...
        case NAND_CB_STATE_3:
            ...
            if (NANDPrivateCreateDirAsync(ConfDirName, NAND_PERM_RWALL, 0, MyNandCallback, &ctrl->commandBlock) !=
                NAND_RESULT_OK) {
                goto _error;
            }
            return;
        ...
        _case_4_lbl:
            ...
            if (NANDPrivateCreateAsync(ConfFileName, NAND_PERM_RWALL, 0, MyNandCallback, &ctrl->commandBlock) !=
                NAND_RESULT_OK) {
                goto _error;
            }
            return;
        ...
        _case_5_lbl:
            ...
            if (NANDPrivateOpenAsync(ConfFileName, &ctrl->fileInfo, NAND_ACCESS_WRITE, MyNandCallback,
        ...
        case NAND_CB_STATE_6:
            ...
            if (NANDWriteAsync(&ctrl->fileInfo, ConfBufForFlush, ctrl->flushSize, MyNandCallback,
                               &ctrl->commandBlock) != NAND_RESULT_OK) {
                goto _error;
            }
            return;
...
static inline bool __SCIsDirty(void) { return DirtyFlag ? true : false; }

static inline void __SCSetDirtyFlag(void) { DirtyFlag = true; }

static inline void __SCClearDirtyFlag(void) { DirtyFlag = false; }

static inline u8* __SCGetConfBuf(void) { return ConfBuf; }

static inline u32 __SCGetConfBufSize(void) { return SC_CONF_MAX_SIZE; }
