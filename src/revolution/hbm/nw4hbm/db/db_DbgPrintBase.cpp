...
namespace nw4hbm {
namespace db {
template <typename charT> MEMiHeapHead* DbgPrintBase<charT>::mHeapHandle;

template <typename charT> DbgPrintBase<charT>* DbgPrintBase<charT>::mInstance;
} // namespace db
} // namespace nw4hbm
...
DbgPrintBase<charT>::DbgPrintBase(ut::Color textColor, ut::Font const* font)
    : mFont(font), mTextColor(textColor), mFontSize(-1.0f), // ?
      mVisible(true) {
    ut::List_Init(&mTextList, offsetof(DbgText, link));
}

template <typename charT> DbgPrintBase<charT>::~DbgPrintBase() {}
...
void DbgPrintBase<charT>::VRegisterf(int x, int y, int time, charT const* format, std::va_list args) {
    ...
    buffer = static_cast<charT*>(MEMAllocFromExpHeapEx(mHeapHandle, bufferSize, alignment));
    if (buffer) {
        int length = ut::TextWriterBase<charT>::VSNPrintf(buffer, bufferSize, format, args);
...
template <typename charT> void DbgPrintBase<charT>::Register(int x, int y, int time, const charT* string, int length) {
    ...
    dbgText = static_cast<DbgText*>(MEMAllocFromExpHeapEx(mHeapHandle, size, alignment));
    if (dbgText) {
        dbgText->x = x;
        dbgText->y = y;
        dbgText->time = time;
...
template <typename charT> void DbgPrintBase<charT>::Flush() {
    ...
    while (dbgText) {
        ...
        if (mVisible) {
            writer.SetCursor(static_cast<f32>(dbgText->x), static_cast<f32>(dbgText->y));
...
template <typename charT> void DbgPrintBase<charT>::SetBuffer(void* buffer, u32 size) {
    mHeapHandle = MEMCreateExpHeap(buffer, size);
}

template <typename charT> void* DbgPrintBase<charT>::ReleaseBuffer() {
    void* addr = nullptr;
...
template <typename charT> void DbgPrintBase<charT>::operator delete(void* ptr) { MEMFreeToExpHeap(mHeapHandle, ptr); }

template <typename charT> void DbgPrintBase<charT>::Registf(int x, int y, const charT* format, ...) {
    std::va_list args;
...
template <typename charT> void DbgPrintBase<charT>::Registf(int x, int y, int time, const charT* format, ...) {
    std::va_list args;
...
template <typename charT>
void DbgPrintBase<charT>::VRegistf(int x, int y, int time, const charT* format, std::va_list args) {
    VRegisterf(x, y, time, format, args);
}

template <typename charT> void DbgPrintBase<charT>::Regist(int x, int y, int time, const charT* string, int length) {
