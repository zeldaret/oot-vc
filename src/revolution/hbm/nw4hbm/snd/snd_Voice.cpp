...
Voice::Voice()
    : mCallback(nullptr), mIsActive(false), mIsStarting(false), mIsStarted(false), mIsPause(false), mSyncFlag(0) {

    for (int i = 0; i < CHANNEL_MAX; i++) {
        for (int j = 0; j < VOICES_MAX; j++) {
            mAxVoice[i][j] = nullptr;
        }
    }
}
...
void Voice::InitParam(int channels, int voices, VoiceCallback pCallback, void* pCallbackArg) {
...
bool Voice::Acquire(int channels, int voices, int priority, VoiceCallback pCallback, void* pCallbackArg) {
    ...
    for (int i = 0; required > i; i++) {
        AxVoice* pAxVoice = AxVoiceManager::GetInstance().AcquireAxVoice(axPrio, AxVoiceCallbackFunc, this);

        if (pAxVoice == nullptr) {
            int rest = required - i;

            const VoiceList& rVoiceList = VoiceManager::GetInstance().GetVoiceList();

            for (VoiceList::ConstIterator it = rVoiceList.GetBeginIter(); it != rVoiceList.GetEndIter(); ++it) {
            ...
            pAxVoice = AxVoiceManager::GetInstance().AcquireAxVoice(allocPrio, AxVoiceCallbackFunc, this);
        }
...
void Voice::AxVoiceCallbackFunc(AxVoice* pDropVoice, AxVoice::AxVoiceCallbackStatus status, void* pCallbackArg) {
...
void Voice::TransformDpl2Pan(f32* pPan, f32* pSurroundPan, f32 pan, f32 surroundPan) {
    surroundPan -= 1.0f;
    ...
    if (ut::Abs(pan) <= ut::Abs(surroundPan)) {
        ...
        if (surroundPan <= 0.0f) {
            *pPan = (0.85f + (1.0f - 0.85f) * (-surroundPan / pan)) * ut::Abs(pan);
            *pSurroundPan = -0.12f + (2.0f * surroundPan + 0.88f * pan);
        } else {
            *pPan = (0.85f + (1.0f - 0.65f) * (-surroundPan / pan)) * ut::Abs(pan);
...
void Voice::CalcMixParam(int channel, int voice, AxVoice::MixParam* pMix, AxVoice::RemoteMixParam* pRmtMix) {
    f32 mainVolume = 0.0f;
    f32 mainSend = 0.0f;
    ...
    if (mOutputLineFlag & OUTPUT_LINE_MAIN) {
        ...
        fxSendA = ut::Clamp(mFxSend[AUX_A] + mVoiceOutParam[voice].fxSend, 0.0f, 1.0f);
    ...
    if (mChannelCount > 1 && mPanMode == PAN_MODE_BALANCE) {
        ...
        switch (AxManager::GetInstance().GetOutputMode()) {
            case OUTPUT_MODE_DPL2: {
                TransformDpl2Pan(&pan, &surroundPan, mPan + voicePan + mVoiceOutParam[voice].pan,
...
void Voice::InvalidateWaveData(const void* pStart, const void* pEnd) {
    ...
    for (int i = 0; i < mChannelCount; i++) {
        ...
        if (pAxVoice != nullptr && pAxVoice->IsDataAddressCoverd(pStart, pEnd)) {
            dispose = true;
            break;
        }
    }
