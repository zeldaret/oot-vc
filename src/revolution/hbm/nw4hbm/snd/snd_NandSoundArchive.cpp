...
class NandSoundArchive::NandFileStream : public ut::NandFileStream {
  public:
    ...
    virtual u32 Tell() const { return ut::NandFileStream::Tell() - mOffset; } // at 0x58

    virtual u32 GetSize() const { return mSize; } // at 0x40
...
ut::FileStream* NandSoundArchive::OpenStream(void* pBuffer, int size, u32 offset, u32 length) const {
    if (!mOpen) {
        return nullptr;
    }
...
ut::FileStream* NandSoundArchive::OpenExtStream(void* pBuffer, int size, const char* pExtPath, u32 offset,
                                                u32 length) const {
    if (!mOpen) {
        return nullptr;
    }
    ...
    NandFileStream* pExtStream = new (pBuffer) NandFileStream(pExtPath, offset, length);
...
NandSoundArchive::NandFileStream::NandFileStream(const char* pPath, u32 offset, u32 size)
    : ut::NandFileStream(pPath, NAND_ACCESS_READ), mOffset(offset), mSize(size) {
