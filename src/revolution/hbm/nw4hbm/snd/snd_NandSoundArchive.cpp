...
bool NandSoundArchive::Open(const char* path) {
    ...
    if (result != NAND_RESULT_OK) {
        NW4HBMAssertWarningMessage_Line(88, result != NAND_RESULT_OK, "Cannot find file %s\n", path);
        return false;
    }
    ...
    if (!bHeader) {
        NW4HBMAssertWarningMessage_Line(97, bHeader, "Cannot load header\n");
        return false;
    }
...
void NandSoundArchive::Close() {
    if (mOpen) {
        ...
        NW4HBMAssertWarningMessage_Line(141, result == NAND_RESULT_OK, "Failed close mcs file. ErrID=%d\n", result);
        mOpen = false;
    }
...
bool NandSoundArchive::LoadHeader(void* buffer, u32 size) {
    ...
    if (size < infoSize) {
        NW4HBMAssertWarningMessage_Line(249, size >= infoSize, "SoundArchive::LoadHeader buffer size is too small.\n");
        return false;
    }
