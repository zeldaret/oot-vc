...
bool SoundThread::Create(s32 priority, void* pStack, u32 stackSize) {
    ...
    BOOL success = OSCreateThread(&mThread, SoundThreadFunc, &GetInstance(), static_cast<u8*>(pStack) + stackSize,
...
void SoundThread::AxCallbackFunc() { GetInstance().AxCallbackProc(); }

void SoundThread::AxCallbackProc() {
    OSSendMessage(&mMsgQueue, reinterpret_cast<OSMessage>(MSG_AX_CALLBACK), 0);

    NW4R_UT_LINKLIST_FOREACH_SAFE(it, mPlayerCallbackList, { it->OnUpdateVoiceSoundThread(); });
...
void SoundThread::SoundThreadProc() {
    ...
    while (true) {
        ...
        if (reinterpret_cast<u32>(msg) == MSG_AX_CALLBACK) {
            ...
            NW4R_UT_LINKLIST_FOREACH_SAFE(it, mSoundFrameCallbackList, { it->OnBeginSoundFrame(); });
            ...
            u32 start = OSGetTick();
            {
                ...
                if (!AxManager::GetInstance().IsDiskError()) {
                    NW4R_UT_LINKLIST_FOREACH_SAFE(it, mPlayerCallbackList, { it->OnUpdateFrameSoundThread(); });
            ...
            NW4R_UT_LINKLIST_FOREACH_SAFE(it, mSoundFrameCallbackList, { it->OnEndSoundFrame(); });

        } else if (reinterpret_cast<u32>(msg) == MSG_SHUTDOWN) {
            NW4R_UT_LINKLIST_FOREACH_SAFE(it, mPlayerCallbackList, { it->OnShutdownSoundThread(); });
