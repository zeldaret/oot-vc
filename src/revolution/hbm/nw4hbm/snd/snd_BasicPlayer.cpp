...
BasicPlayer::BasicPlayer() : mId(BasicSound::INVALID_ID) { InitParam(); }
...
void BasicPlayer::SetRemoteOutVolume(int remote, f32 volume) { mRemoteOutVolume[remote] = volume; }

f32 BasicPlayer::GetRemoteOutVolume(int remote) const { return mRemoteOutVolume[remote]; }

f32 BasicPlayer::GetRemoteSend(int remote) const { return mRemoteSend[remote]; }

f32 BasicPlayer::GetRemoteFxSend(int remote) const { return mRemoteFxSend[remote]; }
