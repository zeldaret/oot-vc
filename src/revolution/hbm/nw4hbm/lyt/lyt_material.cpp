...
{
	std::strncpy(mName, name, ARRAY_COUNT(mName));
}
...
bool Material::SetupGX(bool bModVtxCol, u8 alpha)
{
	...
	else
	{
		...
		for (u8 id = 0; id < tevStageNum; id++)
		{
			...
			GXSetTevColorOp(tevStage, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, 1,
			                GX_TEVPREV);
			GXSetTevAlphaOp(tevStage, GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, 1,
			                GX_TEVPREV);
