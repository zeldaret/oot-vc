...
static u8 DSPInitCode[128] = {
    0x02, 0x9F, 0x00, 0x10, 0x02, 0x9F, 0x00, 0x33, 0x02, 0x9F, 0x00, 0x34, 0x02, 0x9F, 0x00, 0x35, 0x02, 0x9F, 0x00,
...
static inline void waitMicroSec(u32 usec) {
    u32 start = OSGetTick();
    do {
        ;
...
void __OSInitAudioSystem(void) {
    ...
    DSP_HW_REGS[DSP_CSR] = DSP_CSR_HALT | DSP_CSR_AIDINT | DSP_CSR_ARINT | DSP_CSR_DSPINT | DSP_CSR_RES;
    ...
    while (((DSP_HW_REGS[DSP_CPUMBOX_H] << 16) | DSP_HW_REGS[DSP_CPUMBOX_L]) & (DSP_CPUMBOX_H_STATUS << 16)) {
        ;
    }
    ...
    *(u32*)&DSP_HW_REGS[DSP_AR_DMA_CNT_H] = 32;
    ...
    for (ctrl = DSP_HW_REGS[DSP_CSR]; !(ctrl & DSP_CSR_ARINT); ctrl = DSP_HW_REGS[DSP_CSR]) {
        ;
    }
    ...
    *(u32*)&DSP_HW_REGS[DSP_AR_DMA_CNT_H] = 32;
    ...
    for (cpuMboxHi = DSP_HW_REGS[DSP_CPUMBOX_H]; !(cpuMboxHi & DSP_CPUMBOX_H_STATUS);
...
void __OSStopAudioSystem(void) {
    ...
    for (ctrl = DSP_HW_REGS[DSP_CSR]; ctrl & 0x400; ctrl = DSP_HW_REGS[DSP_CSR]) {
        ;
    }

    // Wait for DMA interrupt to end
    for (ctrl = DSP_HW_REGS[DSP_CSR]; ctrl & DSP_CSR_DMAINT; ctrl = DSP_HW_REGS[DSP_CSR]) {
        ;
    }
