...
INIT static void __init_registers(void);
INIT static void __init_data(void);

INIT static void __check_pad3(void) {
    if ((OS_GC_PAD_3_BTN & 0x0EEF) == 0x0EEF) {
        OSResetSystem(0, 0, 0);
    }
}

INIT static void __set_debug_bba(void) { Debug_BBA = true; }

INIT static bool __get_debug_bba(void) { return Debug_BBA; }

INIT WEAK ASM void __start(void) {
...
INIT static ASM void __init_registers(void) {
#ifdef __MWERKS__ // clang-format off
    nofralloc
