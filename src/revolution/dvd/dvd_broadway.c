...
static void initDvdContexts(void) {
    ...
    for (i = 0; i < DVD_LOW_CTX_MAX; i++) {
        dvdContexts[i].callback = nullptr;
...
bool DVDLowClosePartition(DVDLowCallback callback) {
    ...
    result = IOS_IoctlAsync(DiFD, DVD_IOCTL_CLOSE_PARTITION, &diCommand[freeCommandBuf], sizeof(DVDLowDICommand),
                            nullptr, 0, doTransactionCallback, ctx);
...
bool DVDLowReset(DVDLowCallback callback) {
    ...
    result = IOS_IoctlAsync(DiFD, DVD_IOCTL_RESET, &diCommand[freeCommandBuf], sizeof(DVDLowDICommand), nullptr, 0,
...
bool DVDLowSetMaximumRotation(u32 speed, DVDLowCallback callback) {
    ...
    result = IOS_IoctlAsync(DiFD, DVD_IOCTL_SET_MAX_ROTATION, &diCommand[freeCommandBuf], sizeof(DVDLowDICommand),
                            nullptr, 0, doTransactionCallback, ctx);
...
bool DVDLowSeek(u32 offset, DVDLowCallback callback) {
    ...
    result = IOS_IoctlAsync(DiFD, DVD_IOCTL_SEEK, &diCommand[freeCommandBuf], sizeof(DVDLowDICommand), nullptr, 0,
                            doTransactionCallback, ctx);
