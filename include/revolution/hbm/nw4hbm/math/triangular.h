...
namespace nw4hbm {
    namespace math {
        ...
        inline f32 SinIdx(u16 idx) {
            return SinFIdx(NW4HBM_MATH_IDX_TO_FIDX(U16ToF32(idx)));
        }
        inline f32 SinDeg(f32 deg) {
            return SinFIdx(NW4HBM_MATH_DEG_TO_FIDX(deg));
        }
        inline f32 SinRad(f32 rad) {
            return SinFIdx(NW4HBM_MATH_RAD_TO_FIDX(rad));
        }

        f32 CosFIdx(f32 fidx);
        inline f32 CosDeg(f32 deg) {
            return CosFIdx(NW4HBM_MATH_DEG_TO_FIDX(deg));
        }
        inline f32 CosRad(f32 rad) {
            return CosFIdx(NW4HBM_MATH_RAD_TO_FIDX(rad));
        }

        f32 AtanFIdx(f32 x);
        inline f32 AtanDeg(f32 x)  {
            return NW4HBM_MATH_FIDX_TO_DEG(AtanFIdx(x));
        }
        inline f32 AtanRad(f32 x) {
            return NW4HBM_MATH_FIDX_TO_RAD(AtanFIdx(x));
        }

        f32 Atan2FIdx(f32 y, f32 x);
        inline f32 Atan2Deg(f32 y, f32 x)  {
            return NW4HBM_MATH_FIDX_TO_DEG(Atan2FIdx(y, x));
        }
        inline f32 Atan2Rad(f32 y, f32 x) {
            return NW4HBM_MATH_FIDX_TO_RAD(Atan2FIdx(y, x));
        }
    }
}
