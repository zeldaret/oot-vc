...
class Voice : public DisposeCallback {
  ...
  public:
    ...
    void SetVoiceOutParamPitchDisableFlag(bool flag) { mVoiceOutParamPitchDisableFlag = flag; }
    ...
    bool IsPlayFinished() const { return IsActive() && mAxVoice[0][0]->IsPlayFinished(); }
    SampleFormat GetFormat() const;
    u32 GetCurrentPlayingSample() const;
    int GetPhysicalVoiceCount() const { return mChannelCount * mVoiceOutCount; }

    void SetVoiceType(AxVoice::VoiceType type);
    void SetLoopFlag(bool loopFlag);
    void SetLoopStart(int channelIndex, void const* baseAddress, u32 samples);
    ...
    void Stop();
    void StopAtPoint(int channelIndex, void const* baseAddress, u32 samples);
